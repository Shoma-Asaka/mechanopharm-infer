...
def analyze(endpoint_path: str, outdir: str, timecourse_path: str | None = None) -> None:
    out = Path(outdir)
    ...
    write_text_report(out / "report.txt", result=result, reversal=reversal, ec50_df=ec50_df, mopt_df=mopt_df, peak_df=peak_df, final_df=final_df, delayed_df=delayed_df, endpoint_qc=endpoint_qc, timecourse_qc=timecourse_qc)
...
def main() -> None:
    ...
    analyze(endpoint_path=args.endpoint, outdir=args.out, timecourse_path=args.timecourse)
