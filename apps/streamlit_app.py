...
_CALL_CONFIG: dict[str, tuple[str, str]] = {
    "two_state_supported": ("Two-State Architecture Supported", "success"),
    "protected_state_suggested": ("Protected-State Architecture Suggested", "warning"),
    "inconclusive": ("Inconclusive", "info"),
}
_CONFIDENCE_ICON = {"high": "🟢", "moderate": "🟡", "low": "🔴"}
...
def _display_architecture_call(arch: dict) -> None:
    ...
    label, call_type = _CALL_CONFIG.get(call_value, (call_value, "info"))
    conf_icon = _CONFIDENCE_ICON.get(str(confidence), "⚪")

    getattr(st, call_type)(f"**{label}**")

    m1, m2, m3 = st.columns(3)
    m1.metric("Confidence", f"{conf_icon} {confidence}")
...
def _display_results(results: dict[str, Any]) -> None:
    ...
    if "timecourse_panel.png" in results["figures"]:
        st.image(results["figures"]["timecourse_panel.png"], caption="timecourse_panel.png", use_container_width=True)
