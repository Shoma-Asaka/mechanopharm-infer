...
A subsequent patch added three additional diagnostic plots to the standard
output (`dose_response_family.png`, `evidence_summary.png`,
`timecourse_panel.png`) and reworked the Streamlit web interface to persist
results across widget interactions, colour-code the architecture call, surface
supporting evidence and counterpoints as structured lists, and export the
current settings as a reproducible YAML/JSON config.
...
- **Three input modes**: bundled demo data, in-browser table editor, or CSV upload.
- **Advanced settings panel**: bootstrap parameters, assay metadata, and fingerprint
  thresholds (mirrors the CLI `--config` schema).
- **Config export**: the current settings can be downloaded as a YAML (or JSON) file
  compatible with `mechanopharm-infer --config`, enabling reproducible reruns from the
  command line.
- **Persistent results**: analysis outputs remain visible after interacting with any
  widget (e.g. clicking a download button) and are cleared only by pressing *Clear results*.
- **Colour-coded architecture call**: the call label is rendered green
  (`two_state_supported`), amber (`protected_state_suggested`), or blue
  (`inconclusive`), alongside a structured supporting-evidence / counterpoints list
  and interpretation notes.
- **Six diagnostic figures**: `endpoint_landscape`, `ec50_vs_m`, `mopt_vs_c`,
  `dose_response_family`, `evidence_summary`, and `timecourse_panel` (when applicable).
