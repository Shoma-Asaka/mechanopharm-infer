...
- `dose_response_family.png` — dose-response curves by mechanical condition
- `evidence_summary.png` — fingerprint evidence-strength bar chart
- `timecourse_panel.png` (if timecourse is provided) — representative time-course trajectories
...
- `c` : chemical input
- `m` : mechanical input
- `response` : endpoint response (steady-state)
...
Required columns after standardization: `time`, `c`, `m`, `value`.  The
`value` column is the time-resolved counterpart of `response` in the endpoint
table; the two datasets carry overlapping information at the final time point
but serve distinct analytical purposes — endpoint data drives the steady-state
landscape inference (EC50, m*, architecture call), while timecourse data
enables the temporal-dynamics analysis (E_peak, t_peak, delayed protection,
timecourse_panel.png).  Optional columns mirror the endpoint case.
...
A Streamlit interface is available locally via:
...
- **Three input modes**: bundled demo data, in-browser table editor (opens
  blank — enter your own values directly), or CSV upload.
- **Advanced settings panel**: bootstrap parameters, assay metadata, and
  fingerprint thresholds (mirrors the CLI `--config` schema).
- **Config export**: the current settings can be downloaded as a YAML (or
  JSON) file compatible with `mechanopharm-infer --config`, enabling
  reproducible reruns from the command line.
- **Persistent results**: analysis outputs remain visible after interacting
  with any widget (e.g. clicking a download button) and are cleared only by
  pressing *Clear results*.
- **Architecture call display**: the call label and confidence level are
  rendered as plain text alongside a structured supporting-evidence /
  counterpoints list and interpretation notes.
- **Six diagnostic figures**: `endpoint_landscape`, `ec50_vs_m`, `mopt_vs_c`,
  `dose_response_family`, `evidence_summary`, and `timecourse_panel` (when
  applicable), displayed in a proportional grid layout.
- **Downloads**: all outputs as a ZIP or individually.
