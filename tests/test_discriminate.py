...
from mechanopharm_infer.discriminate import build_evidence_flags, build_evidence_table, discriminate_architecture
from mechanopharm_infer.types import QCReport


def test_build_evidence_flags_from_reversal_only():
    reversal = {"low_c_mean_slope": 0.2, "high_c_mean_slope": -0.1, "has_reversal": True, "is_reliable": True, "evidence_strength": "moderate"}
    ec50_df = pd.DataFrame({"m": [0.0, 0.5], "ec50": [0.8, 1.2], "is_reliable": [True, True], "evidence_strength": ["moderate", "strong"]})
...
def test_build_evidence_table_contains_strengths():
    ...
    ec50_df = pd.DataFrame({"m": [0.0, 0.5], "ec50": [0.8, 1.2], "is_reliable": [True, True], "evidence_strength": ["weak", "moderate"]})
    table = build_evidence_table(reversal=reversal, ec50_df=ec50_df)
    assert set(["fingerprint", "supported", "evidence_strength", "source", "notes"]).issubset(table.columns)
...
def test_discriminate_protected_state_suggested():
    reversal = {"low_c_mean_slope": 0.1, "high_c_mean_slope": 0.1, "has_reversal": False, "is_reliable": True, "evidence_strength": "none"}
    mopt_df = pd.DataFrame({"c": [0.5, 1.0, 1.5], "m_opt": [0.4, 0.8, 1.2], "optimum_index": [1, 2, 3], "is_edge": [False, False, False], "is_interior": [True, True, True], "prominence": [0.1, 0.1, 0.1], "is_reliable": [True, True, True], "evidence_strength": ["moderate", "moderate", "strong"]})
    peak_df = pd.DataFrame({"c": [1.0], "m": [0.8], "peak_value": [0.85], "peak_time": [2.0], "peak_index": [1], "is_terminal_peak": [False], "peak_prominence": [0.45], "has_clear_peak": [True], "is_reliable": [True], "warning": [None], "evidence_strength": ["moderate"]})
    final_df = pd.DataFrame({"c": [1.0], "m": [0.8], "e_final": [0.40], "final_time": [3.0], "is_reliable": [True], "warning": [None]})
    delayed_df = pd.DataFrame({"c": [1.0], "m": [0.8], "peak_value": [0.85], "e_final": [0.40], "attenuation": [0.45], "delayed_protection_detected": [True], "is_reliable": [True], "warning": [None], "evidence_strength": ["strong"]})
    endpoint_qc = QCReport(kind="endpoint", passed=True, warnings=[], metrics={})
