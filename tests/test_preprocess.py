...
def test_prepare_endpoint_data_orients_lower_is_stronger_effect():
    df = pd.DataFrame({"c": [0.1, 0.5], "m": [0.0, 0.0], "response": [0.8, 0.2]})
    out = prepare_endpoint_data(df, {"response_mode": "lower_is_stronger_effect"})
    assert np.allclose(out["response"].to_numpy(), [0.2, 0.8])


def test_prepare_endpoint_data_control_subtracted_uses_control_flag():
    df = pd.DataFrame({
        "c": [0.0, 0.5],
        "m": [0.0, 0.0],
        "response": [0.1, 0.5],
        "control_flag": [True, False],
    })
    out = prepare_endpoint_data(
        df,
        {
            "normalization_mode": "control_subtracted",
            "baseline_definition": "control_flag",
        },
    )
    assert np.allclose(out["response"].to_numpy(), [0.0, 0.4])


def test_prepare_timecourse_data_within_mechanics_min_max():
    df = pd.DataFrame({
    ...
    out = prepare_timecourse_data(df, {"normalization_mode": "within_mechanics_min_max"})
    assert np.allclose(out["value"].to_numpy(), [0.0, 1.0, 0.0, 1.0])
