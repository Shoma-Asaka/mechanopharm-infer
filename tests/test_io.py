...
def test_load_endpoint_csv_valid(tmp_path: Path):
    ...
    pd.DataFrame({"c": [0.1, 0.5], "m": [0.0, 0.5], "response": [0.2, 0.7]}).to_csv(path, index=False)
    df = load_endpoint_csv(path)
    assert {"c", "m", "response", "dataset_id", "system", "assay", "control_flag"}.issubset(df.columns)
    assert df.shape[0] == 2


def test_load_endpoint_csv_missing_column(tmp_path: Path):
    path = tmp_path / "bad_endpoint.csv"
    pd.DataFrame({"c": [0.1], "m": [0.0]}).to_csv(path, index=False)
...
def test_load_endpoint_csv_missing_value(tmp_path: Path):
    ...
    pd.DataFrame({"c": [0.1, None], "m": [0.0, 0.5], "response": [0.2, 0.7]}).to_csv(path, index=False)
...
def test_load_timecourse_csv_valid(tmp_path: Path):
    ...
    pd.DataFrame({"time": [0.0, 1.0], "c": [0.5, 0.5], "m": [0.3, 0.3], "value": [0.0, 0.2]}).to_csv(path, index=False)
    df = load_timecourse_csv(path)
    assert {"time", "c", "m", "value", "dataset_id", "system", "assay", "control_flag"}.issubset(df.columns)
    assert df.shape[0] == 2


def test_load_timecourse_csv_missing_column(tmp_path: Path):
    path = tmp_path / "bad_timecourse.csv"
    pd.DataFrame({"time": [0.0], "c": [0.5], "m": [0.3]}).to_csv(path, index=False)
    with pytest.raises(ValueError, match="missing required columns"):
