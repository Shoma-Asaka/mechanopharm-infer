...
def test_example_files_exist():
    ...
    assert (repo_root / "examples" / "demo_endpoint.csv").exists()
    assert (repo_root / "examples" / "demo_timecourse.csv").exists()
...
def test_analyze_end_to_end(tmp_path: Path):
    ...
    assert (outdir / "ec50_bootstrap.csv").exists()
    assert (outdir / "mopt_bootstrap.csv").exists()
    assert (outdir / "fingerprint_evidence.csv").exists()
    assert (outdir / "peak_metrics.csv").exists()
    assert (outdir / "final_response.csv").exists()
    assert (outdir / "delayed_protection.csv").exists()
    assert (outdir / "delayed_protection_bootstrap.csv").exists()
