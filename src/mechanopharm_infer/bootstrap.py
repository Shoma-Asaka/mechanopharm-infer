"""Bootstrap-based uncertainty estimates for response fingerprints.

Each routine resamples the original observations (with replication preserved
per ``(c, m)`` cell or ``(c, m, t)`` cell as appropriate) and propagates the
result into a fingerprint-level CI.  The bootstrap is intentionally light:
its purpose is to flag fragile estimates, not to deliver calibrated coverage.
"""
...
def _aggregate_bootstrap(
    ...
    if not samples:
        return pd.DataFrame(
            columns=[
                key_col,
                f"{value_col}_ci_low",
                f"{value_col}_ci_high",
                f"{value_col}_bootstrap_median",
    ...
    for key, sub in stacked.groupby(key_col, dropna=False, sort=True):
        ...
        support = (
            pd.to_numeric(sub[support_col], errors="coerce").dropna().to_numpy(dtype=float)
            if support_col and support_col in sub.columns
...
def bootstrap_mopt(
    ...
    *,
    ...
    stacked = pd.concat(samples, ignore_index=True)
    interior = (
        stacked.groupby("c", dropna=False, sort=True)["interior_supported"]
        .mean()
        .reset_index()
        .rename(columns={"interior_supported": "interior_optimum_fraction"})
    )
...
def bootstrap_c_rev(
    ...
    *,
    ...
    """Bootstrap CI for the reversal concentration ``c_rev = -Delta_lambda/Delta_mu``.

    Returns a small dict; mergeable into the ``reversal`` payload by callers.
    """

    rng = np.random.default_rng(random_seed)
    estimates: list[float] = []
    deltas_lambda: list[float] = []
    ...
    for _ in range(int(n_boot)):
        ...
        try:
            ...
            continue
        rev = mechanical_sign_reversal(c_grid, m_grid, response)
        if np.isfinite(rev.get("c_rev_estimate", float("nan"))):
            estimates.append(float(rev["c_rev_estimate"]))
        deltas_lambda.append(float(rev.get("delta_lambda_proxy", float("nan"))))
        deltas_mu.append(float(rev.get("delta_mu_proxy", float("nan"))))
    q_low, q_high = _ci_bounds(ci_level)
    if estimates:
        arr = np.asarray(estimates, dtype=float)
        return {
            "c_rev_ci_low": float(np.quantile(arr, q_low)),
            "c_rev_ci_high": float(np.quantile(arr, q_high)),
            "c_rev_bootstrap_median": float(np.median(arr)),
            "c_rev_n_boot_used": int(len(estimates)),
        }
    return {
        "c_rev_ci_low": float("nan"),
        "c_rev_ci_high": float("nan"),
        "c_rev_bootstrap_median": float("nan"),
        "c_rev_n_boot_used": 0,
    }


def bootstrap_delayed_protection(
    ...
    *,
    ...
    for b in range(int(n_boot)):
        ...
        delayed_df = delayed_protection_metrics(
            peak_df, final_df, attenuation_threshold=attenuation_threshold
        ).copy()
        delayed_df["boot_id"] = b
        delayed_df["supported"] = delayed_df.get("is_reliable", False).astype(float)
        delayed_df["delayed_support"] = (
            delayed_df.get("delayed_protection_detected", False).astype(float) * delayed_df["supported"]
        )
