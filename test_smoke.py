...
def test_example_files_exist():
    assert Path("examples/demo_endpoint.csv").exists()
    assert Path("examples/demo_timecourse.csv").exists()
    ...
    pd.DataFrame(
        {
            ...
            "response": [0.2, 0.5, 0.8, 0.3, 0.6, 0.7],
        }
    ...
    pd.DataFrame(
        {
            ...
            "value": [0.0, 0.8, 0.4, 0.0, 0.7, 0.6],
        }
    ).to_csv(timecourse_path, index=False)

    analyze(
        endpoint_path=str(endpoint_path),
        timecourse_path=str(timecourse_path),
        outdir=str(outdir),
    )
